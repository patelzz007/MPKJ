<div class="flex ">
    <form wire:submit.prevent="submit" class="pt-3 w-full">
            ...
            <div class="form-group {{ $errors->has('appointment.user_id') ? 'invalid' : '' }}">
                <label class="form-label required" for="user">{{ trans('cruds.appointment.fields.user') }}</label>
                <x-select-list class="form-control" required id="user" name="user" :options="$this->listsForFields['user']" wire:model="appointment.user_id" />
                <div class="validation-message">
                    {{ $errors->first('appointment.user_id') }}
                </div>
                <div class="help-block">
                    {{ trans('cruds.appointment.fields.user_helper') }}
                </div>
            </div>
